...
3. Run locally:
   ...
   python sales_dashboard_python_project.py --file sales.xlsx --mode kpis

   # Run Streamlit dashboard
   streamlit run sales_dashboard_python_project.py -- --file sales.xlsx --mode streamlit
...
2. Đặt file Excel cạnh file này, hoặc chuyển sang CSV nếu file lớn.
...
- python-dateutil
- scikit-learn
- prophet   # optional for forecasting (install pystan/prophet or prophet)
- openai    # optional, for AI insights


CONTENTS:
- load_data(file_path): robust loader for xlsx/csv; handles large files and type casts
- clean_data(df): cleaning + canonicalization
- compute_kpis(df): returns dict of KPI series/dataframes
- generate_plots(kpis, df): Plotly figures
- export_reports(kpis): save csv/excel
- ai_insights(df, kpis): generate natural-language insights using OpenAI (optional)
- streamlit_app(): interactive dashboard using Streamlit + Plotly
...
def streamlit_app(df: pd.DataFrame = None):
    ...
    if df is None:
        ...
        # Assuming CLI usage isn't the primary issue for this user (they are running streamlit run).
        pass
    ...
    with st.sidebar.expander("🛠 Data Debug Info"):
        ...
        st.write(f"**Raw Columns (Before Clean):** {raw_cols}")
    ...
    # Note: Logic was "Empty = All", but with checkboxes, unchecking all means none.
    # We'll treat empty as none (strict).
    selected_months = checkbox_filter('Select Months', all_months, key_prefix="month_filter")
    # st.sidebar.write("DEBUG: Selected Months", selected_months)
    
    if not selected_months:
         st.warning("Please select at least one month.")
         st.stop() 
    # Keeping it simple: what is checked is selected.
    ...
    # Check if column exists, else skip
    selected_channels = []
    if 'Channel by Sales Person' in df.columns:
        ...
        # If not present, maybe warn or just assume all?
        # Let's assume all if not present to avoid breaking
        pass
        
    # Country Filter (Checkbox)
    # Check if column exists. Note: 'Region' is often used as Country in this dataset based on previous context ("Viet Nam" in Region). 
    # But user specifically asked for "Country" field.
    # If 'Country' column exists, use it. If not, maybe they mean 'Region' is effectively Country?
    # But we already have Region filter.
    # Let's check if 'Country' exists.
    selected_countries = []
    ...
    if single_year_mode:
        ...
        if not single_year_mode:
             comp_label = "Multi-Year View"
        else:
             comp_label = "vs Prev Month"
             # MoM logic omitted for brevity as per original code structure
             pass
        ...
        if has_prev_year:
            ...
            if len(monthly_agg_t) >= 2:
                ...
                if selected_months and len(selected_months) == 1:
                    ...
                    # Let's just say: If no prev year, we disable growth indicators in the "Total" cards,
                    # BUT we add a specific "Monthly Trend" card.
                    pass
    ...
    with tab2:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Volume": st.column_config.NumberColumn(format="%f KG"), # Keep KG suffix if possible? No, %f format doesn't support suffix in printf easily without value. 
                # Actually, NumberColumn format string can be "$%d". So "%d KG" works? 
                # But we styled it. If we style it, it's a string. NumberColumn might interfere.
                # If we use style, we should probably rely on style for suffix too?
                # "{:,.0f} KG"
    ...
    with tab3:
        ...
        # "thêm vào bảng top clients... vị nào họ đang mua... vị nào họ đang không mua trong vòng 6 tháng gần nhất"
        if not df.empty:
             max_d = df[DEFAULT_DATE_COL].max()
             cutoff_gap = max_d - pd.DateOffset(months=6)
             
             # Filter for 6m & Channel (consistent with filtering)
    ...
    with tab4:
        ...
        if not single_year_mode:
            ...
            current_year_val = selected_years[0]
                ...
                if len(selected_months) == 1:
                     try:
                         ...
                         if curr_m_val in all_months:
                             idx = all_months.index(curr_m_val)
                             if idx > 0:
        ...
        if not has_prev_year:
            ...
            def render_yoy_analysis(d_curr, d_prev, label_suffix):
            ...
            if not avail_types:
                ...
                render_yoy_analysis(d_type_curr, d_type_prev, p_type)
    ...
    with tab5:
        ...
        if df_vn_curr.empty:
            ...
            def render_category_focus(cat_name, filter_keywords):
                ...
                # d_top10 contains the Top 10 products for this category.
                # We want to know: For each 'Kind of fruit', which Region dominates?
                ...
                if not d_top10.empty and 'Kind of fruit' in d_top10.columns:
                    ...
                    # For each fruit, find region with highest volume share
                    preferences = []
                    for fruit in pivot_interest.index:
                        ...
                        # Only report if significant preference (>50% maybe? or just simply winner)
                        # Let's say dominant if > 40% share in that region to be safe, or just report "Most popular in..."
                        preferences.append((fruit, winner, pct))
                    
                    # Group by Region to construct sentence
                    # e.g. South: [Mango, Lychee], North: [Strawberry]
                    region_fruits = {}
            ...
            bakery_types = ['Frozen Puree', 'Fruit Filling', 'Frozen Fruit', 'Top&Fill', 'Top & Fill', 'Chocofill'] # Added 'Top & Fill' variant just in case
    ...
    with tab6:
        ...
        if df_curr.empty:
            ...
            # Or simply analyze ALL products in the selection if "Launch" specifically means "New Items"?
            # User said "Product Launching Analysis... compare monthly". 
            # Implies strictly new items. 
            # Logic: Product present in df_curr but NOT in df_prev (if df_prev exists).
            ...
            def show_active_customers(type_cv, kind_cv=None):
                ...
                # We need df_6m_target limited to this pair
                # We can reuse logic or re-filter from df (safer to re-filter to be self-contained)
                
                # 1. Broad Filter 6m
                # Use df_launch (which is filtered) for context or global df?
                # Global df is safer for finding raw transactions.
                ...
                # 3. Group by Client
                # Stats: Total Orders, Total Vol, Last Order
            ...
            # Filter history to currently selected Types/Kinds to determine "Active in Selection" status
            ...
            # Current View Clients (Filtered by Region etc from df_launch)
            current_clients = set(df_launch['Name of client'].unique())
            
            # Intersection: Clients in View WHO ARE Active
            global_unique_cust = len(current_clients)
            global_active_cust = len(current_clients & global_active_clients)
            
            
            # --- Dynamic Region Pivot ---
            # Instead of Macro Region, use Region column directly (which is filtered by Control Panel)
            
            if group_by_type:
                # --- C. Type Level (Broadest) ---
                
                # 1. Sales & Region Pivot
                ...
                # Identify region columns dynamically
                region_cols = [c for c in region_pivot.columns if c != 'Type of product']
                
                # 2. Customer Counts (Per Type, in Selection)
                ...
                # 3. Active Counts (Per Type, in Selection, with Global History Check)
                # A. Get clients present in df_launch per Type
                launch_clients_type = df_launch[['Type of product', 'Name of client']].drop_duplicates()
                
                # B. Determine Active Status per Type (based on 6m history)
                # Filter 6m history to relevant types
                target_types = df_launch['Type of product'].unique()
                df_6m_type = df_6m_raw[df_6m_raw['Type of product'].isin(target_types)]
                
                # Count orders per Type+Client
                ...
                # C. Merge to find intersection
                # We want clients in `launch_clients_type` who exist in `active_pairs`
                ...
                # Reorder columns: Type -> Regions -> Total -> Cust -> Active
                ...
                # Render using st.dataframe with Selection Only (replacing View Button)
                ...
                # Construct Total Row with Global Customer Counts
                ...
                # Ensure correct column order matching final_table
                ...
                # Count Unique Customers per Kind
                ...
                # Active Customers Logic
                # A. Get clients in view
                ...
                sum_cols = region_cols + ['Total']
                total_values = final_table[sum_cols].sum()
                total_row = pd.DataFrame([total_values], columns=sum_cols)
                total_row.insert(0, 'Type of product', 'TOTAL')
                total_row['Kind of fruit'] = ''
                total_row['Name of product'] = ''
                
                # Ensure Column Order for Total Row matches Final Table but with fillers
                # Actually, standard columns: Type, Kind, Product, Regions..., Total
                ...
                # Display Standard Dataframe for Product Level
                st.dataframe(
                    final_table.style.format("{:,.0f}", subset=numeric_cols).apply(highlight_total, subset=['Total']),
                    column_config={
            ...
            with st.expander("View Customer Details for Launched Products"):
                if df_launch.empty:
                    ...
                    # If users filter down to < 5 products, and have multiple years, showing "Product A (2024)" vs "Product A (2025)" is helpful.
            ...
            with c1:
                ...
                with st.expander("Filter Trend Data (Year / Customer)", expanded=False):
                    ...
                    with ft_col1:
                         # 1. Year Filter: Keep as is, based on launch data or global? 
                         # Actually, it's safer to base on global launch data too if we want full consistency, 
                         # but usually year filter is specific. Let's stick to avail_years_trend from df_launch for now 
                         # OR better: use df_potential (filtered by Type/Kind but ALL years) to get available years.
                         # df_potential was defined above as df_curr.copy() which is ALREADY filtered by Year. 
                         # We need a version of df filtered by Type/Kind but NOT by Year for the options.
                    ...
                    with ft_col2:
                         # 2. Key Change: Source options from df_launch_all (Global) instead of df_launch (Year-specific)
                ...
                # We start from df_launch (Current Year) IF we only want to show current year data?
                # NO. The "Trend" chart is supposed to show *Trends* potentially across years?
                # The original code used:
                #    df_trend_src = df_launch.copy() (which comes from df_curr + Filters)
                #    avail_years_trend = sorted(df_launch['Year'].unique()) -> This implies df_launch ONLY has 1 year (curr).
                #    Wait, look at original code: 
                #       df_curr = df[df['Year'] == current_year_val]
                #       df_potential = df_curr.copy() ...
                #       df_launch = df_potential.copy()
                #       avail_years_trend = sorted(df_launch['Year'].unique())
                #    So original logic ONLY showed the CURRENT YEAR in the Trend chart unless 'Year' was somehow not filtered?
                #
                #    Actually, `df_launch` in previous code was derived from `df_potential` which was `df_curr.copy()`. 
                #    So `df_launch` ONLY contained `current_year_val`.
                #    This means the "Monthly Sales Trend" was only showing data for the selected year. 
                #    AND `avail_years_trend` would always have length 1.
                #    So `sel_years_trend` multiselect would never show up or strictly show 1 year.
                # 
                #    IF the user wants to see "Trend", usually they want multiple years? 
                #    BUT, the request is specifically "change Year in control panel, keep filters".
                #    If I am in 2024, I see 2024 trend. I filter Client A.
                #    I switch to 2023. I want to see 2023 trend for Client A.
                #    So, `df_trend_src` SHOULD be based on the NEW `df_launch` (which is 2023 data).
                #    
                #    So, keeping `df_trend_src = df_launch.copy()` is correct for showing the *selected year's* data.
                #    (Unless the user wants multi-year trend in that chart? The original code had `if len(avail_years_trend) > 1`. 
                #     Since df_launch is year-filtered, this check failed unless df_launch somehow had multiple years? 
                #     Ah, `df_curr` in `streamlit_app` comes from `filter_by_date(df, selected_years, ...)`
                #     If `selected_years` (Control Panel) has 1 year, then df_curr has 1 year.
                #     If `selected_years` has multiple years, df_curr has multiple years.
                #     
                #     Assuming Control Panel allows multi-year selection? 
                #     Let's check `tab1`: "Year" selection. 
                #     Default seems to be single year or multi?
                #     Usually Control Panel is Global.
                #     
                #     Anyway, the FIX is to make the FILTERS strict on KEYS and OPTIONS.
                ...
                if df_trend_src.empty:
                    ...
                    # User request: Allow selecting specific products + Select All
                    # 3. Key Change: Options from df_launch_all (Global) filtered by Client
                    if sel_clients_trend:
                        # Optimization: Filter launch_all by client first to narrow products
                ...
                with c2:
                    ...
                    # Filter source data: df_launch contains row-level data.
                    # We need to filter for rows matching the top entities.
                    df_launch_sub = df_launch[df_launch[main_col].isin(top_launch)].copy()
                    
                    # Clean names for chart (Only meaningful for Products usually, but safe to apply or skip)
